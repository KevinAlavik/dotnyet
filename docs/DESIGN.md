...
## Overview
The .NYET bytecode format is a binary format designed for execution by the .NYET virtual machine (VM). It is a stack-based language that supports arithmetic operations, control flow, function definitions, memory operations, and basic I/O. This document specifies the bytecode structure, including the header, opcodes, value types, operand formats, and runtime behavior, enabling developers to generate or interpret .NYET bytecode in any programming language.
...
- **Function Return (`RET`)**:
  ...
  - **Return Value Requirement**: A function must push a value onto the stack before executing `RET`. This value serves as the return value (or return code/item) and is **not** popped by the `RET` instruction. It remains on the stack for the caller to access.
...
- **Stack Operations**: Instructions like `PUSH`, `POP`, `ADD`, and `SUB` manipulate the stack, which holds values of type `Null`, `Integer`, `Double`, `Boolean`, or `String`.
...
### Example Bytecode
Below is an example bytecode sequence for a simple program that defines a `main` function, pushes an integer (42), prints it, and halts:
...
| 14          | `0x01` | Opcode: `PUSH` |
| 15          | `0x01` | ValueTypeTag: `Integer` |
| 16–23       | `0x2A 0x00 0x00 0x00 0x00 0x00 0x00 0x00` | Integer value (42) |
| 24          | `0x50` | Opcode: `PRINT` |
| 25          | `0x40` | Opcode: `HALT` |
...
- **Error Handling**: Validate bytecode bounds before reading operands to prevent out-of-bounds errors.
...
This specification provides all necessary details to generate, parse, or execute .NYET bytecode.